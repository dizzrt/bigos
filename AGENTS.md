...
- `kernel/arch/x86/boot`: Legacy boot sectors, long-mode transition, and ELF loader.
- `kernel/arch/x86/uefi`: x86_64 UEFI loader, PE/COFF handoff assembly, and UEFI
  support headers.
- `kernel/core`: kernel entry and subsystems:
  ...
  - `kernel/core/sched`: multi-core capable scheduler, per-CPU run queues, wait
  ...
  - `kernel/core/fs`: VFS shell over exFAT, RAM-backed `/rw`, persistent
    writable `/rw`, cwd/relative path lookup, constrained rename, metadata,
    fd-backed open/read/write/close/lseek/fsync, writable `bigfs`, block buffer
    cache, and bounded file reads/writes over the block request layer.
...
- `kernel/drivers`: VGA text mode, i8259 PIC, LAPIC/IOAPIC, PIT timer, CMOS RTC,
...
xmake f --ap_startup_percpu_timers=y # BIGOS_AP_STARTUP_PASSED / BIGOS_PERCPU_TIMERS_PASSED
...
xmake f --scheduler_smp_smoke=y # BIGOS_SCHED_SMP_PASSED
xmake f --tlb_shootdown_smoke=y # BIGOS_TLB_SHOOTDOWN_PASSED
xmake f --multicore_hardening_smoke=y # BIGOS_MULTICORE_HARDENING_PASSED
...
xmake f --pci_config_vector_smoke=y # BIGOS_PCI_CONFIG_VECTOR_PASSED
xmake f --pci_msix_smoke=y     # BIGOS_PCI_MSIX_PASSED
xmake f --virtio_blk_smoke=y   # BIGOS_VIRTIO_BLK_PASSED
xmake f --virtio_net_smoke=y   # BIGOS_VIRTIO_NET_PASSED
xmake f --network_protocol_smoke=y # BIGOS_NETWORK_PROTOCOL_PASSED
xmake f --socket_smoke=y       # BIGOS_SOCKET_PASSED
xmake f --modern_storage_backend_smoke=y # BIGOS_MODERN_STORAGE_BACKEND_PASSED
...
xmake f --file_backed_mapping_smoke=y # BIGOS_FILE_BACKED_MAPPING_PASSED/FAILED
xmake f --shared_readonly_mappings_smoke=y # BIGOS_SHARED_READONLY_MAPPINGS_PASSED
xmake f --anonymous_lifecycle_smoke=y # BIGOS_ANONYMOUS_LIFECYCLE_PASSED
xmake f --growable_tables_smoke=y # BIGOS_GROWABLE_TABLES_PASSED
...
xmake f --persistent_writable_fs_smoke=y # BIGOS_PERSISTENT_WRITABLE_FS_PASSED
...
xmake f --sleep_syscall_smoke=y # BIGOS_SLEEP_SYSCALL_PASSED
xmake f --filesystem_maturity_smoke=y # BIGOS_FILESYSTEM_MATURITY_PASSED
xmake f --libc_file_stream_smoke=y # BIGOS_LIBC_FILE_STREAM_PASSED
...
options with `xmake f ...`; `xmake run qemu` launches the default UEFI QEMU
flow, `xmake run qemu -- --display none` launches UEFI QEMU headless, `xmake run
qemu-legacy` selects the Legacy BIOS compatibility flow, `xmake run qemu-gdb`
starts Legacy QEMU paused with the default GDB stub, and `xmake run bochs`
launches the Legacy SDL2 Bochs flow by default. Use `xmake run bochs -- --display
none` for Bochs no-GUI mode.
...
- Prefer QEMU with `--display none` for automated smoke tests, serial marker
  ...
  default UEFI boot validation when Bochs-specific debugging is not required.
- Use Bochs, or Bochs/QEMU cross-validation when available, for early boot,
  ...
  IO, Legacy storage, or hardware-behavior investigations.
...
- For early boot, BIOS, real-mode/protected-mode/long-mode transition, ATA PIO,
  IRQ/timer/syscall, port IO, Legacy storage, or low-level driver behavior, use
  Bochs review or Bochs/QEMU cross-validation when the environment supports it.
...
- Driver port IO and hardware state: `kernel/drivers/video/vga.cc`,
  ...
  `kernel/drivers/block/*.cc`, `kernel/drivers/net/virtio_net.cc`,
  `kernel/drivers/timer/pit.cc`, `kernel/core/bigos/io.cc` (VGA and COM1 serial).
- C++ runtime behavior: `cpp/libsupc++`, `cpp/ktl`, global constructors, and
  `new/delete`.

## Current Maturity

- Boot, VGA/serial output, UEFI framebuffer handoff, glyph/font lookup,
  framebuffer console rendering, kernel-owned IDT and exception/IRQ/syscall
  dispatch, PIC/APIC interrupt delivery, PIT timer IRQ0 with a monotonic tick,
  keyboard decode plus TTY/console input, buddy allocation with an early metadata
  arena, slab allocation (reclaim, large allocations, debug guards, stats),
  kernel virtual memory, owned empty page-table reclamation, and the kernel
  direct map are implemented and exercised by switchable runtime smokes.
- A multi-core capable kernel-thread scheduler with cooperative switching,
  per-CPU state, wait queues, timeout sleep, preemption-disable depth, guarded
  IRQ-return timer preemption, scheduler nudge IPI, remote wakeups, and TLB
  shootdown validation is implemented. The `int 0x80` syscall entry supports the
  diagnostic/user ABI plus bounded process, fd/VFS, pipe/dup, time/identity,
  signal, `brk`, restricted anonymous mapping, bounded socket calls,
  dynamic-link helper calls, and `execve` calls used by the minimal userland.
- The process lifecycle core, growable process/fd tables, safe user-process
  ...
  boundaries where applicable. Bounded async request lifecycle,
