...
- Treat the current incremental userland baseline as multi-core capable and tied to
  ...
  current gaps explicit for full POSIX coverage, complete job control, complete
  ...
  backend parity. Treat those gaps as staged compatibility-expansion targets
  unless a roadmap stage explicitly makes one a non-goal.
- Use bounded wording as an engineering contract for the current capability
  scope, capacity limits, deterministic failures, and validation, not as a
  permanent limit on BigOS becoming a POSIX-compatible Unix-like system.
