...
- To generate and validate a raw image without launching an emulator, run `uv run python tools/boot_debug.py run --no-launch --serial-log log/serial.log`.
- For bounded automated smoke validation, prefer the QEMU headless helper path: `uv run python tools/boot_debug.py run --emulator qemu --display none --serial-log log/serial.log --expect-serial-marker BIGOS_MM_SELF_TEST_PASSED`.
- For Bochs cross-checking, run `uv run python tools/boot_debug.py run --emulator bochs --display none --serial-log log/bochs.serial.log --expect-serial-marker BIGOS_MM_SELF_TEST_PASSED` when local Bochs ROM/display configuration is available.
