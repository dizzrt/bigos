...
| `multicore-hardening` | `--multicore_hardening_smoke=y` | `BIGOS_MULTICORE_HARDENING_PASSED` | 25s | Default-off QEMU legacy 2-CPU hardening path for AP scheduler placement, scheduler nudge IPI, remote wakeup, timeout wakeup, TLB shootdown completion, and normal userland baseline marker; not CPU hotplug, NUMA, async I/O, full POSIX concurrency, or UEFI runtime parity. |
...
| `block-io-request-layer` | `--block_io_request_smoke=y` | `BIGOS_BLOCK_IO_REQUEST_PASSED` | 20s | Bounded synchronous request submission, validation failure, per-device queue exhaustion, unsupported write, device-error propagation, and kernel-internal interrupt-driven completion; no complete async I/O or user ABI. |
| `bounded-network-protocol` | `--network_protocol_smoke=y` | `BIGOS_NETWORK_PROTOCOL_PASSED` | 20s | Kernel-internal bounded Ethernet II, ARP, IPv4, ICMP echo, and UDP datagram path; no socket, fd, syscall, device node, DHCP, DNS, TCP, IPv6, or complete network stack is claimed. |
| `minimal-socket-interface` | `--socket_smoke=y` | `BIGOS_SOCKET_PASSED` | 20s | Kernel-internal closed loop over the minimal user-visible UDP socket fd backend and the bind/sendto/recvfrom path with injected frames: socket fd creation, ops identity, unsupported `read`/`write`, bind, send, receive, no-data and oversize error paths, and exactly-once endpoint reclamation on close. Bounded UDP subset only; no complete POSIX socket API, TCP/stream, `connect`/`listen`/`accept`, option matrix, DHCP, DNS, IPv6, or real tap/network backend is claimed, and recvfrom is bounded (not general POSIX blocking). |
| `modern-storage-backend` | `--modern_storage_backend_smoke=y` | `BIGOS_MODERN_STORAGE_BACKEND_PASSED` | 30s | Explicit modern virtio-blk backend selection through the request layer, cache-mediated write/read round-trip, and writeback failure dirty retention; default boot remains ATA-backed and no crash consistency, power-loss recovery, device node, or async I/O is claimed. |
...
| `journal-recovery` | `--journal_recovery_smoke=y` | `BIGOS_JOURNAL_RECOVERY_PASSED` | 40s | M15.2 mount-time journal recovery validation: clean mount, committed replay, repeated replay idempotence, partial discard, corrupt reject, and recovery I/O failure fail-closed behavior. |
...
| `libc-file-stream` | `--libc_file_stream_smoke=y` | `BIGOS_LIBC_FILE_STREAM_PASSED` | 40s | Bounded buffered `FILE` stream subset: `fopen`/`freopen`/`fclose`, buffered `fread`/`fwrite` round-trip, `fgets`, `fseek`/`ftell`/`rewind`, `setvbuf` modes plus caller buffer, `fflush`/`feof`/`ferror`/`clearerr`, exit-path flush, `freopen(stdout)` redirect, deterministic failure paths, and expanded `string`/`stdlib`/`ctype` helpers. |
...
| Bounded network protocol path | `bounded-network-protocol` smoke, static IPv4 context, controlled Ethernet frames | ARP cache/reply, IPv4 validation, ICMP echo reply, UDP bind/send/receive, and unsupported-frame rejection complete with `BIGOS_NETWORK_PROTOCOL_PASSED` | Missing marker, protocol failure marker, malformed packet accepted, queue/capacity behavior mismatch, or unexpected user ABI exposure | Default-off kernel-internal protocol assertion | xmake, cross-binutils, QEMU serial log; TAP/virtio-net host backend only when packet-injection evidence is selected |
| Minimal UDP socket interface | `minimal-socket-interface` smoke, static IPv4 context, socket fd backend plus injected datagram | Socket fd creation, ops-identity, unsupported `read`/`write`, bind, sendto, recvfrom with correct source address/payload, no-data (`EAGAIN`-mapped) and oversize (`EMSGSIZE`-mapped) error paths, and exactly-once endpoint reclamation on close complete with `BIGOS_SOCKET_PASSED` | Missing marker, socket failure marker, wrong payload/source, leaked or double-freed endpoint, or unexpected blocking behavior | Default-off kernel-internal socket assertion | xmake, cross-binutils, QEMU serial log; no real tap/network backend required |
...
The runner writes a Markdown-first validation artifact to `logs/runtime-smoke-validation.md` unless `--output` is provided. The artifact keeps JSON schema compatible fields for future automation:
