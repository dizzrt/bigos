...
- `uv run python tools/boot_debug.py run --boot-mode uefi --emulator qemu --display none --image build/test/uefi-esp.img --uefi-root-image build/test/uefi-root.raw --serial-log log/qemu-uefi.serial.log --expect-serial-marker BIGOS_USER_EXEC --smoke-timeout 40` is the direct helper form.
...
- The glyph lookup asset currently records Unifont 8x16 half-width and 16x16 full-width bitmap glyphs. Runtime console output decodes bounded UTF-8 into console-owned Unicode codepoint cells and uses this width class to place half-width and full-width glyphs. The framebuffer backend may render those cells with a software cursor, while the Legacy VGA text backend deterministically degrades non-ASCII cells. This remains a bounded default console feature and does not turn the default console into a full graphical, ANSI/VT, locale-aware, shaping, or POSIX terminal.
