...
`xmake run bochs-sdl2` 和 `xmake run bochs`。
...
`xmake run bochs-sdl2` 和 `xmake run bochs` 保持现有的 Legacy BIOS/MBR/exFAT/Bochs 含义。
这些入口不会切换为 `BOOTX64.EFI`、ESP/FAT 镜像或 QEMU/OVMF。
