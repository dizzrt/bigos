...
- `x86_64-elf-g++ -std=c++17 -ffreestanding ... -fsyntax-only cpp/ktl/bitset.cc cpp/ktl/buffer.cc cpp/ktl/list.cc kernel/mm/slab.cc kernel/mm/buddy.cc kernel/mm/vmem.cc kernel/mm/kmem.cc`：通过。
- `clang++ --target=x86_64-elf -std=c++17 -ffreestanding ... -fsyntax-only cpp/ktl/bitset.cc cpp/ktl/buffer.cc cpp/ktl/list.cc kernel/mm/slab.cc kernel/mm/buddy.cc kernel/mm/vmem.cc kernel/mm/kmem.cc`：通过。
- `x86_64-elf-g++` 和 `clang++` 对包含 `algorithm`、`allocator`、`queue`、`priority_queue`、`rb_tree`、`map` 的聚焦头文件检查：通过。
- `xmake`：已运行，但当前全量构建在 `kernel/core/irq/isr.cc` 的既有 `irq_handler`/`MAX_IRQ_NUM` 诊断处失败，未到达链接和 Bochs smoke test。
