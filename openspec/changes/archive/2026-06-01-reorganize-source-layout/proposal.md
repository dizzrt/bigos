...
当前 BigOS 的实现代码分散在仓库顶层的 `arch/`、`kernel/`、`mm/`、`drivers/`、`cpp/` 和 `lib/kernel/` 中，随着 boot、IRQ、内存管理、驱动和 KTL 等子系统继续增长，根目录会越来越像源码目录而不是项目入口目录。现在整理项目组织结构，可以在不改变内核运行时行为的前提下，建立更清晰的源码边界，降低后续新增子系统、构建配置和文档维护的成本。

## What Changes

- 新增统一的实现源码布局约定：内核、驱动、内存管理、架构相关 boot 代码和运行时启动对象源码收敛到 `kernel/` 下。
...
- 影响 C++ 支撑库边界：顶层 `cpp/`、`cpp/include`、`cpp/ktl` 和 `cpp/libsupc++` 保留为独立 C++ support library，不迁入 `kernel/` 或顶层 `include/`。
