...
- [x] 1.4 建立目标 `kernel/`、`cpp/`、`include/` 和 `tools/` 目录结构规划，明确 `kernel/core`、`kernel/mm`、`kernel/drivers`、`kernel/arch/x86/boot`、`kernel/runtime`、顶层 `cpp`、`cpp/include`、`cpp/ktl`、`cpp/libsupc++` 和 `tools` 的职责边界。

## 2. 迁移低风险内核实现源码

- [x] 2.1 将 `kernel` 实现源码迁移到 `kernel/core`，保持 namespace、符号名、include 形式和初始化流程不变。
- [x] 2.2 将 `drivers` 实现源码迁移到 `kernel/drivers`，保持 `driver::*` namespace、VGA 行为、PIC 行为、port IO 顺序和 visible failure 行为不变。
- [x] 2.3 将 `mm` 实现源码和实现私有头迁移到 `kernel/mm`，保持 buddy、slab、kmalloc、vmem 的初始化顺序、对象生命周期、对齐规则和失败行为不变。
- [x] 2.4 更新 `xmake.lua` 对 `kernel/core`、`kernel/drivers`、`kernel/mm` 的源码收集和 include search path 配置，移除对应 stale top-level source roots。
- [x] 2.5 更新 `.clangd` 中与 `kernel`、`drivers`、`mm` 相关的 include path，确保 public include 不需要写入 `kernel/` 前缀。
...
- [x] 4.1 将 `arch/x86/boot` 迁移到 `kernel/arch/x86/boot`，保持 MBR/DBR、long-mode transition、ELF64 加载和 boot handoff 协议不变。
...
- [x] 5.1 将 `lib/src` 迁移到 `kernel/runtime`，保持 `crt0.s`、`crti.s`、`crtn.s` 内容语义和输出 object 名称不变。
...
- 目录规划与迁移结果：`kernel/core`、`kernel/drivers`、`kernel/mm`、`kernel/arch/x86/boot`、`kernel/runtime`、`tools/install.py` 已建立；顶层 `cpp/`、`cpp/include`、`cpp/ktl`、`cpp/libsupc++` 和顶层 `include/` 保持独立边界。
- 构建配置：`xmake.lua` 已改为从 `kernel/core`、`kernel/drivers`、`kernel/mm` 收集 kernel target 源码，并保留顶层 `cpp/**.cc`；boot 源码仍通过 `kernel/arch/x86/boot/Makefile` 独立构建，避免被误链接进 kernel target；`.clangd` 使用顶层 `include`、`cpp/include`、`cpp/libsupc++/include`，不再暴露 `kernel/drivers/include`、`kernel/mm/include` 或全局 `kernel/mm`。
- Runtime startup：`make -C kernel/runtime` 通过，生成 `lib/crt0.o`、`lib/crti.o`、`lib/crtn.o`；`xmake.lua` 中 `crt0`、`crti`、`crtbegin`、`crtend`、`crtn` 的链接顺序未改变；`link.lds` 未修改，高半区地址 `0xffffffff80000000`、`.init`、`.fini`、`.ctors`、`.dtors` 收集规则未改变。
- Boot 路径：`make -C kernel/arch/x86/boot build-mbr build-dbr build-exdbr build-boot` 通过；`mbr.s` 和 `dbr_exfat.s` 仍有历史 assembler warning：`found movsd; assuming movsl was meant`；未修改 boot 地址、磁盘布局、`kernel` ELF 文件名、高半区跳转地址或 `BootInfo` 布局。
- 主构建：`xmake` 已运行，能从 `kernel/` 新路径编译源码；构建在 `kernel/core/irq/isr.cc` 的历史 `irq_handler`/`MAX_IRQ_NUM` 诊断处失败，未到达链接和 Bochs smoke test；该诊断与归档验证记录中的旧 `kernel/irq/isr.cc` 问题一致，不是本次路径迁移新增。
- C++/assembly 辅助检查：`x86_64-elf-g++ ... -fsyntax-only` 对 `cpp/ktl`、`cpp/libsupc++`、`kernel/mm`、`kernel/drivers`、`kernel/core/bigos`、`kernel/core/kernel.cc`、`kernel/core/irq/interrupt.cc` 通过；`clang++ --target=x86_64-elf ... -fsyntax-only` 报告历史 noexcept 声明/定义不一致（`kernel/mm/memory.cc`、`kernel/drivers/irqchip/i8259.cc`、`kernel/core/irq/interrupt.cc`），与路径迁移无关，未在本 change 中改变运行时代码语义。
