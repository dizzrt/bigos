...
- 受影响子系统：`src/kernel/kernel.cc` 初始化尾部、`src/kernel/timer`、`src/kernel/irq`、新增 `src/kernel/sched` 或等价调度器目录、`include/bigos` public scheduler/thread headers、`xmake.lua` 源文件注册、`tests` 源码级验证与 `docs/arch` 架构说明。
