...
阶段 5 已完成用户地址空间页表准备与 `int 0x80` syscall 入口，BigOS 已具备进入用户态前的两块关键边界：可派生低半区独立的用户页表根，以及可由软件触发的受控内核入口。下一步需要把这些基础串成最小闭环：构造一个用户进程，加载第一个内置用户程序，切换到 ring3，并通过 `write`/`exit` 类最小 syscall 回到内核。

当前内核仍停留在 ring0 内核线程模型；阶段 5 的 specs 明确禁止切 CR3、进入 ring3 或加载用户程序。阶段 6 需要用一个专门 change 放宽这些阶段性禁令，同时保持 boot、内存、中断、调度和诊断 ABI 稳定。
...
- 扩展 syscall 使用边界：阶段 5 已定义的 `int 0x80` ABI 需要允许从 ring3 触发，`SYS_DEBUG_WRITE`/`SYS_GET_TICK` 之外补齐最小 `write`/`exit` 语义或将诊断 syscall 收敛为该闭环可用的最小 ABI。
...
- `user-address-space-vmem`: 将阶段 5 的“派生用户页表根但不得切 CR3/进入 ring3”约束扩展为“默认准备 API 不切换，但阶段 6 进程运行路径 MAY 激活派生根并运行用户映射”，并固定激活/恢复边界。
- `syscall-entry`: 将阶段 5 的“仅 ring0 自测、IDT gate DPL=0、不从用户态触发 syscall”扩展为允许 ring3 通过受控 `int 0x80` 入口进入内核，并补充用户指针、长度、返回值和 `exit` 语义。

## Impact

- 受影响子系统：`src/kernel/sched` 或等价调度路径、`src/kernel/irq` 的 IDT/syscall/#PF 处理、`src/kernel/sys` syscall dispatch、`src/mm/vmem` 用户地址空间 primitive、`src/kernel/kernel.cc` 初始化与 smoke wiring、`xmake.lua` 构建开关、`tests` 源码级检查和 `docs/arch` 架构文档。
