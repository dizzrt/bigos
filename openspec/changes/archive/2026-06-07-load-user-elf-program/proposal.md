...
kernel block and filesystem read capability 已经把只读 block device 与 exFAT 文件读取能力下沉到内核运行时，但user entry and syscall capability 的首个用户程序仍来自 flat embedded image，无法验证“磁盘文件 -> ELF program headers -> 用户地址空间 -> ring3”的真实加载链路。现在需要把这条链路作为user ELF loader capability 的独立 OpenSpec change 固化下来，为后续 exec、多进程和更完整的用户程序产物管理提供基础。
...
- 保留user entry and syscall capability flat embedded user program smoke 作为独立回归路径；本 change 不移除现有 embedded smoke。
...
- `first-user-program`: 明确user entry and syscall capability 的 flat embedded 首个用户程序 smoke 保持可用，同时允许新的 ELF loader runtime 复用最小 `Process`、ring3 entry、syscall 和 safe teardown 边界，而不把文件系统依赖并入原 embedded smoke。
...
- 依赖能力：ring0 syscall diagnostic capability syscall entry、user entry and syscall capability first user program runtime、user entry and syscall capability.5 address-space lifecycle、kernel block and filesystem read capability block/fs read。
