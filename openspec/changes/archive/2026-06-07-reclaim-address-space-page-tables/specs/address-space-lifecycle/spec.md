...
BigOS SHALL provide a user address-space teardown helper that releases resources owned by a terminated or faulted user process: user leaf physical pages, user low-half dynamic page-table pages, the user PML4 root, and the process kernel stack when it is no longer the current execution stack. The helper MUST preserve borrowed kernel high-half mappings and MUST run only in a safe kernel context.

#### Scenario: teardown 只遍历用户 owned 低半区

- **WHEN** BigOS tears down a user address-space root
...
- **WHEN** teardown removes a process-owned user code, data, BSS, or stack mapping
...
- **WHEN** a terminated or faulted process is being reaped
...
BigOS SHALL separate user termination from resource reclamation. `SYS_EXIT`, user-mode page fault handling, and invalid user-buffer handling MUST mark the current process terminated or faulted and arrange a later safe teardown; they MUST NOT immediately free the current kernel stack, active user root, or process object on the same unsafe return path.

#### Scenario: SYS_EXIT 标记待回收

- **WHEN** the first user program invokes `SYS_EXIT`
- **THEN** BigOS MUST record the exit code and mark the process terminated or reap-pending
...
- **THEN** BigOS MUST record a deterministic fault reason and mark the process faulted or reap-pending
...
- **WHEN** a reap-pending process is selected for teardown
- **THEN** BigOS MUST execute teardown from non-IRQ context with a safe kernel address-space root active
- **AND** it MUST verify that the resources being freed are not currently required by the executing stack or CR3 state
