...
**Reason**: 项目调试入口将统一到 xmake，继续要求 `make boot-debug` 稳定会与移除 Makefile 的目标冲突。

**Migration**: 使用 `xmake f ...` 配置 smoke 开关，并使用 `xmake run bochs-sdl2` 启动 Legacy BIOS/MBR/exFAT/Bochs SDL2 调试路径。
