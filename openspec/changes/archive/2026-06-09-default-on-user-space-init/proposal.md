...
构建里被偶发验证，而非默认路径上被持续运行。behavior assertion validation baseline 是后续所有 POSIX 工作（demand
...
- 在默认构建（不加任何 smoke 开关）中产出 `BIGOS_INIT_ENTER` / `BIGOS_INIT_EXIT`
  serial marker，并把该默认 case 纳入 runtime smoke validation matrix runtime smoke 矩阵。
...
- 启动「行为断言测试」轨道：新增基于 serial marker + 用户态二进制输出的行为断言用例，作为
  runtime smoke validation matrix 矩阵的演进起点（持续推进，非一次性完成）。
...
- `runtime-smoke-validation`: runtime smoke validation matrix 矩阵新增一个**默认构建**（无 smoke 开关）case，
...
- 受影响子系统：kernel 入口（`kernel/core/kernel.cc`）、proc（init 启动 / 退出语义，
  ...
  打包流程（`tools/boot_debug.py` 与磁盘镜像安装）、runtime smoke validation matrix smoke 矩阵与验证脚本。
