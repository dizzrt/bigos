...
- [x] 1.4 修改 `page_fault_handler`（[interrupt.cc](file:///Users/bytedance/Desktop/workspace/kernel/bigos/src/kernel/irq/interrupt.cc#L42-L61)）调用统一入口；用户态恢复失败 -> `fault_current_and_exit`，CPL0 缺页保持 `BIGOS_PAGE_FAULT` 诊断 + halt
- [x] 1.5 回归现有 `user_program_smoke` / `user_elf_smoke` / `fs_smoke`，确认仅切换入口不改变行为

## 2. 匿名 backing 惰性物化

- [x] 2.1 将 `map_anonymous_current`（[proc.cc](file:///Users/bytedance/Desktop/workspace/kernel/bigos/src/kernel/proc/proc.cc#L1280-L1341)）改为仅登记非重叠 VMA 与 `anon_next`，不再 eager 逐页分配映射；保持 unsupported 请求确定性拒绝与无部分发布
- [x] 2.2 将 `brk_current` 扩展（[proc.cc](file:///Users/bytedance/Desktop/workspace/kernel/bigos/src/kernel/proc/proc.cc#L1255-L1278)）改为仅更新 `heap_break` 与堆 VMA 边界元数据，不 eager 映射；扩展失败仅元数据回滚保持旧边界
