...
fork/exec process capability 已落地 `fork_current`（COW 复制地址空间、复制 fd 表），fork 子进程目前继承大部分父进程语义但没有身份字段可继承。signal capability（信号）需要「谁能 kill 谁」的判定，可写文件系统与 pipe/dup foundation（可写文件系统）需要 owner/mode 与文件时间戳——两者都依赖本阶段提供的墙钟与身份原语。

约束：freestanding、单核、同步、无 libc；CMOS RTC 经端口 0x70（index）/0x71（data）访问，是可写文件系统与 pipe/dup foundation 之外唯一新引入的硬件访问点；syscall 走 `int 0x80`，寄存器 ABI 与现有号位固定，只能在末尾追加。
...
- 提供两个纯判定原语：进程间特权操作判定（未来 kill 的基础）与文件 owner/mode 访问判定（可写文件系统与 pipe/dup foundation 的基础），本阶段只提供判定逻辑、不接线任何强制点。
...
- 不做文件时间戳写回、信号实际交付、可写文件系统强制点——这些是signal capability/18 的工作；本阶段只交付它们消费的原语。
...
- 理由：在没有 login/setuid 的前提下，全系统只有 root 一个身份，字段存在的价值是为signal capability/18 提供「可继承、可判定」的结构位，而非现在就区分多用户。备选「单独的 cred 结构体 + 引用计数」对当前单一身份是过度设计，否决。
...
- 理由：本阶段没有信号、没有可写文件，没有任何地方需要强制；提前把判定逻辑做成纯函数可单测、可被signal capability/18 直接复用，避免事后补权限。强制点接线明确留给消费阶段。
...
- 备选：exec 时刷新为当前墙钟 —— 会把「镜像替换时刻」与「进程创建时刻」混淆，且对未来 shell（`fork`+`exec`）观察子进程起始时间不利，否决。如shell 与用户态组合能力 有不同期望再单独评估。

### 决策 8：文件 `mode`/`Access` 常量本阶段放进公共头

文件权限位常量与 `Access` 枚举本阶段就放进公共头（如 `include/bigos/cred.h` 或合并进 proc 公共头），供可写文件系统与 pipe/dup foundation 可写文件系统直接引用。

- 理由：本阶段已实现 `permits(...)` 纯判定函数，其入参类型（mode 位、Access）天然是其公共契约的一部分；与判定函数同处公共头可避免可写文件系统与 pipe/dup foundation 再搬迁或重复定义。保持最小导出面：仅导出常量、枚举与判定函数签名，不导出内部实现细节。
- 备选：先放 proc 内部、可写文件系统与 pipe/dup foundation 再上提 —— 会导致一次无谓的搬迁与可能的重复定义，否决。
...
- [身份字段当前全是 root，等于没有真正的多用户隔离] → 明确这是为signal capability/18 预留的结构位；判定函数已正确实现 root/非 root 分支，待 login/setuid 落地即生效，无需重构。
...
- 无。原先的三个待定项已收敛为决策 6（`current_unix_time()` 返回 `int64_t`）、决策 7（exec 不刷新 `start_unix_time`）、决策 8（文件 `mode`/`Access` 常量本阶段放进公共头）。如shell 与用户态组合能力 shell 对子进程起始时间有不同期望，可再单独评估决策 7。
