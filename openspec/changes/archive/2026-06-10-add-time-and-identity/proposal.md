...
- 受影响代码：[timer.h](include/bigos/timer.h) 或新增 `include/bigos/time.h`（墙钟 API）、新增 RTC 驱动头/源（`include/drivers/rtc/*`、`src/drivers/rtc/*`）、[proc.h](include/bigos/proc.h)（`Process` 身份/时间字段与权限判定声明）、[proc.cc](src/kernel/proc/proc.cc)（身份初始化/继承、权限判定实现）、[syscall.h](include/bigos/syscall.h) 与 [syscall.cc](src/kernel/syscall/syscall.cc)（新增 syscall 号与分支）。
