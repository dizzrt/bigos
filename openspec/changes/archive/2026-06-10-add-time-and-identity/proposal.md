...
- 受影响子系统：`src/drivers`（新增 CMOS RTC 一次性读取驱动）、`src/kernel/timer` 或新增 `src/kernel/time`（墙钟基准 + 当前墙钟查询）、`src/kernel/proc`（`Process` 身份字段、init/fork 身份初始化与继承、权限判定原语）、`src/kernel/syscall`（新增只读身份/时间 syscall 分发）。
- 受影响代码：[timer.h](file:///Users/bytedance/Desktop/workspace/kernel/bigos/include/bigos/timer.h) 或新增 `include/bigos/time.h`（墙钟 API）、新增 RTC 驱动头/源（`include/drivers/rtc/*`、`src/drivers/rtc/*`）、[proc.h](file:///Users/bytedance/Desktop/workspace/kernel/bigos/include/bigos/proc.h)（`Process` 身份/时间字段与权限判定声明）、[proc.cc](file:///Users/bytedance/Desktop/workspace/kernel/bigos/src/kernel/proc/proc.cc)（身份初始化/继承、权限判定实现）、[syscall.h](file:///Users/bytedance/Desktop/workspace/kernel/bigos/include/bigos/syscall.h) 与 [syscall.cc](file:///Users/bytedance/Desktop/workspace/kernel/bigos/src/kernel/syscall/syscall.cc)（新增 syscall 号与分支）。
