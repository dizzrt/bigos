...
- 受影响代码：[syscall.h](include/bigos/syscall.h)（新增 `SYS_FORK`）、[syscall.cc](src/kernel/syscall/syscall.cc)（dispatch 分支）、[proc.h](include/bigos/proc.h)（`fork_current` 声明、COW/引用计数相关结构与 `try_handle_user_page_fault` 扩展）、[proc.cc](src/kernel/proc/proc.cc)（地址空间/VMA/fd 复制、COW 缺页分支、确定性回滚）、[memory.h](include/bigos/memory.h) 与 `src/mm`（用户物理帧引用计数与按根复制页表的辅助 API）。
