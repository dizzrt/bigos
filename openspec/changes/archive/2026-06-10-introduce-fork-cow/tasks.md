...
- [x] 1.2 在 `src/mm` 新增按物理帧号索引的引用计数表（直接映射区定长 `uint16_t` 数组，上界取最高可分配物理帧号），新增 `init_frame_refcount()` 并接在 [kmem.cc](src/mm/kmem.cc) `init_mem` 末尾、`init_direct_map` 之后；定义计数饱和为确定性失败（回滚 `fork`）而非回绕（决策 8）。
...
- [x] 2.6 在 [proc.cc](src/kernel/proc/proc.cc) 的 `try_handle_user_page_fault` 内、present 位判定处新增 COW 写分裂分支：写访问命中 present、只读、`PTE_COW`、可写匿名 VMA 时进入 `cow_split_current`；其余 present 违例维持原 `return false`(kill)。
...
- [x] 3.2 在 [syscall.cc](src/kernel/syscall/syscall.cc) 的 `dispatch` 新增 `SYS_FORK` 分支：校验非 IRQ / 可分配 / 有当前 Running 进程的上下文，调用 `proc::fork_current()` 并把返回值写回 `frame->rax`；该路径不发 i8259 EOI、不放宽其它 gate、不改寄存器约定。
