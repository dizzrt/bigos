...
- 阶段 16.5 已在 `Process` 中加入 uid/gid/euid/egid，并实现了纯判定原语 `bigos::cred::may_signal(actor, target)`（见 [cred.cc](file:///Users/bytedance/Desktop/workspace/kernel/bigos/src/kernel/proc/cred.cc#L6-L16)），但目前没有任何强制点调用它。
...
- IRQ dispatch（[interrupt.cc](file:///Users/bytedance/Desktop/workspace/kernel/bigos/src/kernel/irq/interrupt.cc#L132-L166)）在外部 IRQ handler 返回并发送 EOI 后，调用 `bigos::sched::maybe_preempt_on_irq_return(__frame)`；该路径已通过 `(__frame->cs & 0x3) == 0x3` 区分用户态/内核态被中断帧（见 page_fault_handler）。这正是 POSIX 信号「在返回用户态边界投递」的天然挂载点。
