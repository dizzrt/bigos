...
- 受影响代码：新增 `include/bigos/signal.h` 与 `src/kernel/signal/*`；[syscall.h](include/bigos/syscall.h) 与 [syscall.cc](src/kernel/syscall/syscall.cc)（新增号位与分支）；[interrupt.cc](src/kernel/irq/interrupt.cc)（IRQ-return 用户态边界投递点）；[proc.h](include/bigos/proc.h) 与 [proc.cc](src/kernel/proc/proc.cc)（信号字段、初始化/继承/重置、默认动作终止、`SIGCHLD`）；[cred.h](include/bigos/cred.h) / [cred.cc](src/kernel/proc/cred.cc)（`may_signal` 接线，判定逻辑不变）；[errno.h](include/bigos/errno.h)（如需 `ESRCH` 等错误码补齐）。
