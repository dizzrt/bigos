...
- 受影响子系统：新增 `src/kernel/signal`（信号核心：pending/mask/处置表管理、kill 投递、IRQ-return 投递与信号帧构造、sigreturn 恢复）、`src/kernel/syscall`（新增 4 个 syscall 分发分支）、`src/kernel/irq`（IRQ-return 到用户态边界增加信号投递检查点）、`src/kernel/proc`（`Process` 信号字段、init/ELF 初始化、fork 继承、exec 重置、默认 Terminate 接入 teardown、`SIGCHLD` 投递）、`src/kernel/sched`（与 IRQ-return/exit 生命周期协作）。
- 受影响代码：新增 `include/bigos/signal.h` 与 `src/kernel/signal/*`；[syscall.h](file:///Users/bytedance/Desktop/workspace/kernel/bigos/include/bigos/syscall.h) 与 [syscall.cc](file:///Users/bytedance/Desktop/workspace/kernel/bigos/src/kernel/syscall/syscall.cc)（新增号位与分支）；[interrupt.cc](file:///Users/bytedance/Desktop/workspace/kernel/bigos/src/kernel/irq/interrupt.cc)（IRQ-return 用户态边界投递点）；[proc.h](file:///Users/bytedance/Desktop/workspace/kernel/bigos/include/bigos/proc.h) 与 [proc.cc](file:///Users/bytedance/Desktop/workspace/kernel/bigos/src/kernel/proc/proc.cc)（信号字段、初始化/继承/重置、默认动作终止、`SIGCHLD`）；[cred.h](file:///Users/bytedance/Desktop/workspace/kernel/bigos/include/bigos/cred.h) / [cred.cc](file:///Users/bytedance/Desktop/workspace/kernel/bigos/src/kernel/proc/cred.cc)（`may_signal` 接线，判定逻辑不变）；[errno.h](file:///Users/bytedance/Desktop/workspace/kernel/bigos/include/bigos/errno.h)（如需 `ESRCH` 等错误码补齐）。
