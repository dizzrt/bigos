...
当前默认用户态已经能启动 resident init 并进入 `/bin/sh`，但默认运行时文本控制台仍未被明确提升为可见、可操作的交互入口。交互控制台可用性需要把有界 shell 的 prompt、键盘输入回显和命令输出整理成项目级可用性目标，同时继续保留串口/日志可验证路径，避免把交互体验推进为完整 POSIX terminal。
...
- `runtime-smoke-validation`: 扩展验证需求，要求 交互控制台可用性的交互控制台改动保留 headless 串口/日志行为断言，并记录无法自动化验证的人工控制台风险。
