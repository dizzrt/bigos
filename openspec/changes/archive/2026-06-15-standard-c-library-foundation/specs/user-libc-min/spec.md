...
- **WHEN** 文档、规格、头文件注释或用户程序说明描述 Stage 40 libc
...
- **WHEN** 审查 Stage 40 后的用户态 libc 头文件集合
...
### Requirement: Stage 40 高价值 C 库函数

BigOS 用户态 libc SHALL 优先补齐或稳定简单静态程序高频使用的 C 库函数。Stage 40 第一批接口 MUST include `calloc`、`realloc`、`strtol`、`atoi` 和 bounded `snprintf`。新增或整理的函数 MUST 保持 freestanding-safe，MUST 不依赖宿主 runtime，MUST 在有界输入下具有确定性行为，并且 MUST 在失败时遵循接口约定而不破坏调用者已有状态。`calloc` MUST 检查乘法溢出并返回清零内存；`realloc` MUST 在失败时保留原块有效；`strtol` MUST 支持有界整数解析、base 处理和 end pointer；`atoi` MUST 作为十进制简化 wrapper；`snprintf` MUST 遵守输出缓冲区边界并复用 bounded formatter。

#### Scenario: 新增函数具备实现和规格

- **WHEN** Stage 40 向 public headers 新增一个 C 库函数声明
...
- **WHEN** 一个 Stage 40 静态 C 程序包含对应 public headers 并调用 `calloc`、`realloc`、`strtol`、`atoi` 或 `snprintf`
...
BigOS 用户态 libc SHALL 在 Stage 40 中保持 stdio 为 fd-backed bounded helper。`stdin`、`stdout`、`stderr` MAY 使用最小 opaque `FILE` 表示，但 public behavior MUST 限定在 standard streams、`read`/`write` wrapper、格式化输出和 deterministic error reporting。Stage 40 bounded formatter MUST support the existing minimal formats plus common width and integer/pointer forms, at least `%u`、`%p`、`%ld`、`%lu` and `%zu`, and MUST share behavior across `printf`、`fprintf(stderr, ...)` and bounded `snprintf` where applicable. Stage 40 stdio MUST NOT require `fopen`、`fclose`、`fread`、`fwrite`、`fflush`、完整 buffering、locale、宽字符、浮点 formatting、完整 flags 或完整 precision。
...
- **WHEN** 审查 Stage 40 stdio 相关 public headers
...
- **THEN** libc MUST 按 Stage 40 bounded formatter 规则产生确定性文本
...
- **WHEN** 一个普通 Stage 40 静态 C 程序只包含 general libc umbrella header 或标准 C/POSIX-like public headers
...
- **WHEN** 一个 Stage 40 静态 C 程序通过 `DIR*` 风格 wrapper 打开并读取一个支持目录
...
- **WHEN** 文档、headers 或 validation notes 描述 Stage 40 `DIR*` 风格 wrapper
...
### Requirement: Stage 40 libc 验证分层

BigOS SHALL 为 Stage 40 C 库基础提供分层验证路径。验证 MUST 覆盖 public header 可构建性、errno 翻译、成功调用不改写 errno、字符串/内存例程、分配失败语义、stdio/error reporting、只读环境访问、文件/进程 wrapper 和至少一个代表性静态用户程序消费路径。依赖 QEMU、Bochs、x86_64 cross toolchain、显示/ROM 依赖或磁盘镜像配置的运行时检查 MAY 被跳过，但跳过记录 MUST 说明缺失条件、已运行替代检查和残余风险。

#### Scenario: libc subset smoke 覆盖核心行为

- **WHEN** Stage 40 libc subset runtime validation 在配置完整的 emulator 环境中运行
