...
BigOS SHALL 实现有界三次握手连接建立，覆盖主动打开（发送 SYN → `SYN_SENT` → 收 SYN,ACK → 发 ACK → `ESTABLISHED`）与被动打开（`LISTEN` → 收 SYN → 发 SYN,ACK → `SYN_RECEIVED` → 收 ACK → `ESTABLISHED`）。连接建立 MUST 在 ordinary（可阻塞、非 IRQ）内核上下文推进，MUST 正确初始化双向序号空间，并对非法或不匹配的握手段以确定性状态处理。
...
- **THEN** BigOS MUST 发送 SYN,ACK 进入 `SYN_RECEIVED`，在收到匹配 ACK 后进入 `ESTABLISHED`
- **AND** MUST 正确初始化双向序号空间并递增确定性连接建立诊断计数
