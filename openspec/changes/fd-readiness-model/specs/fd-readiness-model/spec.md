...
### Requirement: 终端描述符就绪桥接到 tty 输入

终端（tty）描述符的就绪查询 SHALL 桥接到全局 tty 输入路径，复用既有输入可用判断：当 tty 输入环中存在可消费输入（或存在挂起的转义序列字节）时 MUST 报告可读；终端写出方向 MUST 报告可写；终端输入路径不产生错误位。该桥接 MUST 仅做只读查询，不消费输入记录。

#### Scenario: 有输入的终端报告可读

- **WHEN** tty 输入环中存在尚未消费的输入记录，查询终端描述符就绪
