...
- **WHEN** a request fails before device execution because it is invalid, the target device queue is full, the target device is not ready, or completion state cannot be armed
...
- **THEN** they MUST identify it as a bounded kernel-internal interrupt-driven completion model for block requests
