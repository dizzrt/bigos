...
定义 BigOS 有界多路复用能力的用户可见契约：向简单静态用户程序提供一个
poll 风格的多路复用 syscall，对一个定容描述符集合带毫秒级超时等待，复用统一
fd 就绪模型（`vfs::poll_file`）逐项报告可读/可写/错误就绪，并复用调度等待队列
在无就绪时真正阻塞让出、任一描述符就绪或超时到期再唤醒。该能力保持有界，不声称
完整 POSIX `poll`/`select`/`epoll`/`ppoll` 语义、不支持无界或动态增长的描述符
集合、不引入边缘触发、事件通知对象、带外/优先级事件或信号中断的复杂 restart 语义。
