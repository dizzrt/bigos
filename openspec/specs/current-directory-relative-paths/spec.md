...
### Requirement: runtime filesystem maturity 文件系统操作共享 cwd 解析
BigOS SHALL route runtime filesystem maturity path-taking filesystem operations through the shared bounded cwd-relative path resolution contract. Operations including open, stat, mkdir, unlink, restricted regular-file rename, and directory enumeration setup MUST treat absolute paths as root-relative and relative paths as current-directory-relative with supported `.` and `..` components. This requirement MUST NOT introduce symlink traversal, mount namespaces, `chroot`, stable inode identity, or complete POSIX pathname canonicalization.
