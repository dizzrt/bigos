...
BigOS SHALL provide each process with a bounded file descriptor table. The table MUST allocate descriptors deterministically, map descriptors to open file objects, reject invalid descriptors, and release references when descriptors are closed or the process is reaped.

#### Scenario: open 分配最低可用 fd

- **WHEN** a process opens a file and its fd table has capacity
...
- **WHEN** a process opens a file but the fd table has no available entry
- **THEN** BigOS MUST fail open deterministically and MUST release any unpublished open file object
...
- **WHEN** a process reads or closes an fd that is unused, already closed, outside table bounds, or not readable
