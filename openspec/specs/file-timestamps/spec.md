...
定义 BigOS 有界文件时间戳模型，覆盖用户可见的 `atime`、`mtime`、`ctime` 秒级字段、可写后端更新时间戳的规则、显式更新时间接口，以及 `stat`/`touch` 等用户态消费路径。该能力只承诺 BigOS 当前 VFS、bigfs `/rw` 后端、metadata ABI、syscall ABI 和 freestanding libc 支持的最小子集，不引入纳秒级精度、时区/locale 格式化、符号链接时间戳、目录 fd 相对接口或完整 POSIX timestamp 语义。
