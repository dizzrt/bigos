...
定义 BigOS 可增长、可回收的进程注册结构与每进程 fd 表能力：以可配置软上限取代
编译期固定的 `MAX_PROCESSES`/`MAX_FDS` 硬上限，用内核堆分配（`kmalloc`/`free`）
承载 `Process` 对象与 fd 存储，并随进程生命周期分配、unpublish 与回收。该能力
单核安全，不引入 SMP、命名空间、进程组、会话语义，也不改变默认 boot 行为。
