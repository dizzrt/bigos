...
Define the BigOS kernel direct-map capability: an explicit higher-half virtual
address window for covered ordinary physical RAM, separate from the kernel
heap/vmalloc area, recursive page-table self-mapping, low identity mappings, and
the higher-half kernel image mapping.
