...
内核虚拟内存分配器 SHALL 按请求页数分配不重叠的 kernel virtual ranges，并 MUST 在分配、释放和合并后保持 free/used list 与 `nr_free_pages_` 一致。页表映射 MUST 使用正确的 x86_64 四级页表 9 位索引。kernel virtual pages 分配接口 MUST 使用页数语义，physical buddy 分配接口 MUST 使用 order 语义。
