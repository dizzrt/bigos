...
BigOS SHALL provide a switchable early memory runtime self-test that runs after `init_mem()` and before later kernel subsystems when explicitly enabled for validation builds.
...
- **THEN** the kernel runs memory self-test coverage before initializing later subsystems such as IRQ/PIC
