...
定义 BigOS 内核内部“本机地址”loopback 网络路径：在有界网络协议路径的 IPv4 输出层识别本机
IPv4 地址与回环网段 `127.0.0.0/8` 目的，将其直接闭环回本机 IPv4 输入分发路径，无需构造
Ethernet 帧、无需 ARP 解析、无需帧级网络设备发送。该能力复用既有 UDP 端点有界队列、就绪
模型与 ICMP 校验，支持本机 UDP 闭环与 ICMP echo-to-self 有界闭环，并保持对外（非本机）目的
的既有 `route_destination` + ARP + 帧级设备发送行为不变。它不引入通用 IP routing/转发、多
接口地址模型、`lo` 设备节点或新的用户可见 syscall/fd 语义。
