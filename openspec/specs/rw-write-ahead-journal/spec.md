# rw-write-ahead-journal Specification

## Purpose

Define BigOS persistent `/rw` write-ahead journaling for the journal-capable bigfs format. This capability covers bounded in-volume journal layout, journal-first transaction ordering, deterministic failure behavior, conservative pre-recovery mount validation, and default-off validation boundaries. It does not claim mount-time replay, crash recovery, power-loss safety, automatic repair, or changes to the default RAM-backed `/rw` behavior.
