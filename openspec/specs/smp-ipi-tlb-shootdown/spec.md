...
Define the bounded SMP IPI and cross-CPU TLB shootdown capability for BigOS: typed internal IPI delivery, IRQ-safe IPI handlers, address-space residency based target selection, ordered shootdown completion, and reproducible validation while keeping CPU hotplug, NUMA, broad APIC external IRQ migration, and user-visible ABI changes out of scope.
