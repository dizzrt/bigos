...
定义 BigOS 墙钟时间能力：在保留现有 `bigos::timer` 单调 tick 的前提下，从 CMOS RTC 一次性读取基准时间（UTC、BCD/二进制模式判断、字段校验），转换为 Unix epoch 秒，并以现有单调 tick 推进当前墙钟；提供只读的当前墙钟查询 API；定义 RTC 无效时的确定性退化语义；并经默认关闭的验证开关验证。该能力不引入持续 RTC 轮询、时钟同步/校准、时区/夏令时/闰秒或高精度计时器。
