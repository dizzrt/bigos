...
Define the runnable x86_64 UEFI boot backend spike for BigOS. This capability
covers the PE/COFF UEFI loader artifact, ESP/FAT image generation,
QEMU/OVMF smoke entry, BootInfo v2 handoff production, conservative UEFI memory
map normalization, optional boot metadata, and isolation from the existing
Legacy BIOS backend.
