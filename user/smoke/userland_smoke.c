...
static void test_smoke_shell(char **envp) {
    ...
    unlink("/rw/smoke_core_boot.txt");
    unlink("/rw/smoke_core_cat.txt");
    unlink("/rw/smoke_core_cp.txt");
    unlink("/rw/smoke_core_wc.txt");
    unlink("/rw/smoke_core_a.txt");
    unlink("/rw/smoke_core_b.txt");
    unlink("/rw/smoke_core_c.txt");
    unlink("/rw/smoke_core_dir/touched.txt");
    rmdir("/rw/smoke_core_dir");
    ...
    write_all_or_exit(input[1], "cat /boot/fs_smoke.txt > smoke_core_boot.txt\n");
    write_all_or_exit(input[1], "write smoke_core_a.txt alpha beta\n");
    write_all_or_exit(input[1], "append smoke_core_a.txt gamma delta\n");
    write_all_or_exit(input[1], "cp smoke_core_a.txt smoke_core_b.txt\n");
    write_all_or_exit(input[1], "cat smoke_core_b.txt > smoke_core_cat.txt\n");
    write_all_or_exit(input[1], "wc smoke_core_b.txt > smoke_core_wc.txt\n");
    write_all_or_exit(input[1], "grep beta smoke_core_b.txt\n");
    write_all_or_exit(input[1], "echo pipeline-ok | grep pipeline\n");
    write_all_or_exit(input[1], "head -n 1 smoke_core_b.txt\n");
    write_all_or_exit(input[1], "tail -n 1 smoke_core_b.txt\n");
    write_all_or_exit(input[1], "hexdump smoke_core_b.txt\n");
    write_all_or_exit(input[1], "stat smoke_core_b.txt\n");
    write_all_or_exit(input[1], "mkdir smoke_core_dir\n");
    write_all_or_exit(input[1], "touch smoke_core_dir/touched.txt\n");
    write_all_or_exit(input[1], "truncate 0 smoke_core_dir/touched.txt\n");
    write_all_or_exit(input[1], "mv smoke_core_b.txt smoke_core_c.txt\n");
    write_all_or_exit(input[1], "rename smoke_core_c.txt smoke_core_b.txt\n");
    write_all_or_exit(input[1], "ls smoke_core_dir\n");
    write_all_or_exit(input[1], "find smoke_core_dir\n");
    write_all_or_exit(input[1], "du smoke_core_dir\n");
    write_all_or_exit(input[1], "rm smoke_core_dir/touched.txt\n");
    write_all_or_exit(input[1], "rmdir smoke_core_dir\n");
    write_all_or_exit(input[1], "cp smoke_core_b.txt /boot/smoke_core_bad.txt\n");
    write_all_or_exit(input[1], "grep -E beta smoke_core_b.txt\n");
    write_all_or_exit(input[1], "grep beta smoke_core_b.txt missing_core_input.txt > smoke_core_cp.txt\n");
    ...
    require_file_contains("/rw/smoke_shell_io.txt", "rmdir: smoke_shell_path_dir/nested: rmdir: Directory not empty");
    require_file_contains("/rw/smoke_shell_io.txt", "stat: smoke_shell_path_dir/nested: stat: No such file or directory");
    require_file_contains("/rw/smoke_shell_io.txt", "status 1");
    require_file_contains("/rw/smoke_core_boot.txt", "BIGOS_FS_SMOKE_PAYLOAD");
    require_file_contains("/rw/smoke_core_cat.txt", "alpha beta");
    require_file_contains("/rw/smoke_core_cat.txt", "gamma delta");
    require_file_contains("/rw/smoke_core_wc.txt", "2 4");
    require_file_contains("/rw/smoke_shell_io.txt", "alpha beta");
    require_file_contains("/rw/smoke_shell_io.txt", "pipeline-ok");
    require_file_contains("/rw/smoke_shell_io.txt", "alpha beta");
    require_file_contains("/rw/smoke_shell_io.txt", "gamma delta");
    require_file_contains("/rw/smoke_shell_io.txt", "00000000");
    require_file_contains("/rw/smoke_shell_io.txt", "path=smoke_core_b.txt type=file");
    require_file_contains("/rw/smoke_shell_io.txt", "cp: /boot/smoke_core_bad.txt: open: Read-only file system");
    require_file_contains("/rw/smoke_shell_io.txt", "grep: unsupported option: -E");
